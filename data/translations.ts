export const translations = {
  fr: {
    // Header
    "nav.about": "À propos",
    "nav.projects": "Projets",
    "nav.contact": "Contact",

    // Hero Section
    "hero.badge": "Account Executive • AI Enthusiast",
    "hero.greeting": "Bonjour, je suis",
    "hero.name": "Paul-Antoine Sage",
    "hero.description":
      "Account Executive passionné par l'IA. Je construis des cas d'usage concrets pour maîtriser l'intelligence artificielle dans un cadre professionnel et apporter de la valeur mesurable aux entreprises réelles.",
    "hero.cta.projects": "Voir les cas d'usage",
    "hero.cta.contact": "Me contacter",
    "hero.status": "Disponible pour échanger",

    // About Section
    "about.title": "À propos",
    "about.subtitle":
      "De l'Account Executive à l'AI Enthusiast - Créer de la valeur concrète avec l'IA",
    "about.mission.title": "🎯 Ma Mission",
    "about.mission.text":
      "En tant qu'Account Executive, j'ai découvert le potentiel immense de l'IA pour transformer les processus métier. Ma mission est d'apprendre à maîtriser l'IA en créant des cas d'usage concrets qui apportent une valeur mesurable aux entreprises réelles.",
    "about.approach.title": "💼 Mon Approche",
    "about.approach.text":
      "Plutôt que d'apprendre l'IA en théorie, je construis des projets professionnels qui résolvent de vrais problèmes business. Chaque projet est un laboratoire d'apprentissage où je teste, itère et mesure l'impact concret de l'IA sur l'efficacité opérationnelle.",
    "about.stack.title": "🛠️ Stack IA & Tech",
    "about.focus.title": "🌟 Focus Métier",
    "about.focus.1": "Automatisation des processus de vente",
    "about.focus.2": "Analyse intelligente de conversations",
    "about.focus.3": "Optimisation du cycle de vente avec l'IA",
    "about.focus.4": "ROI mesurable et KPIs concrets",

    // Projects Section
    "projects.title": "Mes Cas d'Usage IA",
    "projects.subtitle":
      "Projets concrets pour maîtriser l'IA et créer de la valeur pour des entreprises réelles",
    "projects.more": "Voir plus",
    "projects.less": "Voir moins",
    "projects.features": "Fonctionnalités clés",
    "projects.detailed": "Description détaillée",
    "projects.impact": "Impact mesurable",
    "projects.links": "Liens",
    "projects.demo": "Démo live",
    "projects.github": "Code source",
    "projects.alpic": "Voir sur Alpic",

    // Contact Section
    "contact.title": "Contact",
    "contact.subtitle":
      "Discutons de vos défis business et de l'impact que l'IA peut avoir",
    "contact.heading": "Échangeons sur vos projets IA",
    "contact.text":
      "Vous avez un défi business qui pourrait bénéficier de l'IA ? Vous voulez automatiser vos processus de vente ? Parlons-en et trouvons ensemble des cas d'usage concrets !",
    "contact.cta": "Envoyer un email",

    // Footer
    "footer.rights": "Tous droits réservés.",
  },
  en: {
    // Header
    "nav.about": "About",
    "nav.projects": "Projects",
    "nav.contact": "Contact",

    // Hero Section
    "hero.badge": "Account Executive • AI Enthusiast",
    "hero.greeting": "Hi, I'm",
    "hero.name": "Paul-Antoine Sage",
    "hero.description":
      "Account Executive passionate about AI. I build concrete use cases to master artificial intelligence in a professional setting and deliver measurable value to real businesses.",
    "hero.cta.projects": "View use cases",
    "hero.cta.contact": "Contact me",
    "hero.status": "Open to a chat",

    // About Section
    "about.title": "About",
    "about.subtitle":
      "From Account Executive to AI Enthusiast - Creating concrete value with AI",
    "about.mission.title": "🎯 My Mission",
    "about.mission.text":
      "As an Account Executive, I discovered the immense potential of AI to transform business processes. My mission is to learn to master AI by creating concrete use cases that bring measurable value to real businesses.",
    "about.approach.title": "💼 My Approach",
    "about.approach.text":
      "Rather than learning AI in theory, I build professional projects that solve real business problems. Each project is a learning laboratory where I test, iterate and measure the concrete impact of AI on operational efficiency.",
    "about.stack.title": "🛠️ AI & Tech Stack",
    "about.focus.title": "🌟 Business Focus",
    "about.focus.1": "Sales process automation",
    "about.focus.2": "Intelligent conversation analysis",
    "about.focus.3": "AI-powered sales cycle optimization",
    "about.focus.4": "Measurable ROI and concrete KPIs",

    // Projects Section
    "projects.title": "My AI Use Cases",
    "projects.subtitle":
      "Concrete projects to master AI and create value for real businesses",
    "projects.more": "See more",
    "projects.less": "See less",
    "projects.features": "Key Features",
    "projects.detailed": "Detailed description",
    "projects.impact": "Measurable Impact",
    "projects.links": "Links",
    "projects.demo": "Live demo",
    "projects.github": "Source code",
    "projects.alpic": "View on Alpic",

    // Contact Section
    "contact.title": "Contact",
    "contact.subtitle":
      "Let's discuss your business challenges and the impact AI can have",
    "contact.heading": "Let's talk about your AI projects",
    "contact.text":
      "Do you have a business challenge that could benefit from AI? Want to automate your sales processes? Let's discuss and find concrete use cases together!",
    "contact.cta": "Send an email",

    // Footer
    "footer.rights": "All rights reserved.",
  },
};

export type TranslationKey = keyof typeof translations.fr;

